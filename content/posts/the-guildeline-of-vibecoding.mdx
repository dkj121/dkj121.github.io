...
## 一、何为 [vibecoding](https://zh.wikipedia.org/wiki/Vibe_coding)

> vibe 情景式 coding 编码

自从[OpenAI](https://openai.com/) 发布了 [Codex](https://openai.com/zh-Hans-CN/codex/) 之后，AI 编程助手的概念就被广泛传播开来。
...
## 二、参考[最佳实践](https://zh.wikipedia.org/wiki/%E6%9C%80%E4%BD%B3%E5%AE%9E%E8%B7%B5)
...
### 2.[Codex](https://openai.com/zh-Hans-CN/codex/)
...
> 并在 Codex 的设置中启用 WSL2 支持。
> ![image3](../assets/the-best-practice-of-vibecoding/image3.png)
...
> 除了以下内容 claude code 官方也提供了大量优秀的 [plugins](https://github.com/anthropics/claude-plugins-official)、[skills](https://github.com/anthropics/skills)
...
> 尽管当前大多数 ai 能够自主的调用大部分拓展，但是这里依然还是建议开发者在开发过程中主动声明已有的拓展，以便于 ai 进行更好的调用（实际上对于ai不会主动调用的拓展反而是不成功的）
> 同时对于添加到项目中的拓展，提前确认拓展内容，避免与其他拓展产生冲突（常见于前端设计拓展）
> 对于自己生成的 skills 一般仅分三种情况：
> 1. 当前项目的固定 workflow
> 2. 文档、图片生成（大多数前端 skills 都包含）
> 3. 强调利用 MCP （大多数 MCP 提供方都会同时提供对应 skills）
> 并且严格按照：`What it does` + `When to use it` + `Key capabilities` 格式进行描述

## 五、Quickly Start (normal)

> 下文利用 skills 代替具体命令，仅供参考
...
3. 适当的使用 `/mattpocock-skills:handoff`、`/undo`、`/clear` 清理无用的上下文
4. 对于前端修改，先利用 [figma](https://www.figma.com/)、[pen](https://www.pen.dev/)、[penpot](https://penpot.app/) 等工具先生成样例文件，待最终确定后再开始编码
5. 对于体系分明的项目，在开发过程中，适当使用 `/skill-creator` 等 skill 生成指令或者使用 `/find-skill`来生成或找到对应的 skill
6. 即时覆写 ai 写好的 plan 文件

> 此外，对于多 agents 并行，claude code 采用了不同的策略 [Run agents in parallel](https://code.claude.com/docs/en/agents) 来适配各种项目
> 至于更多提示词细节可参考：[best-practices-for-prompt-engineering](https://claude.com/blog/best-practices-for-prompt-engineering)、[codex_prompting_guide](https://developers.openai.com/cookbook/examples/gpt-5/codex_prompting_guide)
...
ai 本质上作为一个使用工具的工具，每一次对话都是在从头构建一套工具包（上下文），在每一次对话都是在为当前项目设计一套工具包（上下文）。

> 即 [context engineering](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)

对于小玩具来说或许没必要如此讲究，但是对大多数人来说，不光是为了提高效率，更是为了在未来的开发中，能够更好的适应 ai 的发展。

> 对于更多工程实践，可以前往 [Engineering at Anthropic](https://www.anthropic.com/engineering)、[Stories about the technology and builders at OpenAI.](https://openai.com/zh-Hans-CN/news/engineering/)
